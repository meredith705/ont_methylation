...
task pepperMarginDeepVariantHaplotag {
    input {
        ...
        Int memSizeGB = 128
        Int threadCount = 64
        Int diskSizeGB = 4 * round(size(inputReads, 'G')) + round(size(assembly, 'G')) + 100
    ...
    output {
        File interOut      = "pepper_deepvariant_output/intermediate_files/PHASED.PEPPER_MARGIN.haplotagged.bam"
